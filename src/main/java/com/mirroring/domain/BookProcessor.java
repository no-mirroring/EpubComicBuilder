    ...
    private void processOPF() {
        ...
        for (int i = 0; i < chapterList.size(); i++) {
            ...
            switch (chapter.getMediaType()) {
                //图片章节
                case MediaType.CHAPTER_PIC_LIST:
                    ...
                    break;
                //文字章节
                case MediaType.CHAPTER_TEXT:
                    break;
                //html章节
                case MediaType.CHAPTER_HTML:
                    List<File> picList1 = chapter.getResourceHtml().getPicFileList();
                    for (int j = 0; j < picList1.size(); j++) {
                        File pic = picList1.get(j);
                        //图片添加时放在images/
                        manifest.addItem(manifest.new Item("image_" + FilenameUtils.getBaseName(pic.getName()),
                                "images/" + pic.getName(),
                                MediaType.MANIFEST_JPEG));
